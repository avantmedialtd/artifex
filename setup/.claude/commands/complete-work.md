...
4. Run the following command: `af commit save "<issue-summary>" Issue=<issue-key> OpenSpec-Id=<openspec-id>` where `<issue-summary>` is derived from the Jira issue and `<openspec-id>` is the change ID found in step 2.
