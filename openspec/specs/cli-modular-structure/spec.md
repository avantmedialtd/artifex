...
#### Scenario: Claude availability check is shared utility

- **GIVEN** multiple commands need to check Claude Code availability
- **WHEN** the codebase is organized
- **THEN** the Claude availability check is in a shared utility module (e.g., utils/claude.ts)
- **AND** can be imported by any command that needs it
- **AND** is not duplicated across command modules

#### Scenario: Output utilities are shared

- **GIVEN** multiple commands need consistent output formatting
- **WHEN** the codebase is organized
- **THEN** output utilities are in a shared module (e.g., utils/output.ts)
- **AND** can be imported by any command
- **AND** provide consistent formatting across the CLI
