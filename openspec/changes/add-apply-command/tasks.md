...
- [x] Add `handleSpecApply()` function to [commands/spec.ts](../../../commands/spec.ts) following the pattern of `handleSpecArchive()` and `handleSpecPropose()`
...
- [x] Add unit tests for `handleSpecApply()` in a new test file [spec-apply.test.ts](../../../spec-apply.test.ts)
...
- [x] Update [integration.test.ts](../../../integration.test.ts) to include apply command tests
...
- [x] Run `npm run format` to ensure code formatting
- [x] Run `npm run lint` to check for linting issues
- [x] Run `npm run spell:check` to verify spelling
- [x] Run `npm test` to ensure all tests pass
- [x] Manually test `zap apply` and `zap spec apply` commands
- [x] Validate with `openspec validate add-apply-command --strict`
